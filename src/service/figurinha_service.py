...
class FigurinhaService:
    ...
    def create_figurinha(self, dados_requisicao: dict) -> Figurinha:
        # campos obrigatórios
        if not dados_requisicao.get("numero") or not dados_requisicao.get("tipo") or not dados_requisicao.get("posicao"):
            raise ValidationError("Todos os campos (numero, tipo, posicao) são obrigatórios.")
        ...
        if tipo not in self.tipos_validos:
            raise ValidationError(f"Tipo inválido. Deve ser um de: {self.tipos_validos}")
        if posicao not in self.posicoes_validas:
            raise ValidationError(f"Posição inválida. Deve ser uma de: {self.posicoes_validas}")
    ...
    def get_todas_figurinhas(self, filtro_posicao: Optional[str] = None, filtro_tipo: Optional[str] = None) -> List[Figurinha]:
        # validar a categoria tanto no filtro da listagem
        if filtro_posicao and filtro_posicao not in self.posicoes_validas:
            raise ValidationError(f"Filtro de posição inválido. Deve ser uma de: {self.posicoes_validas}")
            
        if filtro_tipo and filtro_tipo not in self.tipos_validos:
            raise ValidationError(f"Filtro de tipo inválido. Deve ser um de: {self.tipos_validos}")
    ...
    def put_figurinha(self, figurinha_id: int, dados_atualizacao: dict) -> Figurinha:
        ...
        if not dados_atualizacao.get("numero") or not dados_atualizacao.get("tipo") or not dados_atualizacao.get("posicao"):
            raise ValidationError("Todos os campos (numero, tipo, posicao) são obrigatórios.")
